...
const componentLine = (doc, component) => {
    ...
    const type = component.type === "percentage"
        ? `${percentageValue ?? 0}%`
        : component.type === "slab"
            ? "slab"
...
export const generatePayslipPdf = (payload, res) => {
    ...
    const totalWorkingDays = Number(period.working_days || summary.total_days || payroll.total_days || 0);
    ...
    const attendanceValue = (compactKey, summaryKey, payrollKey) => attendance[compactKey] ?? summary[summaryKey] ?? payroll[payrollKey] ?? "-";
    const totals = {
    ...
    res.setHeader("Content-Disposition", `inline; filename=payslip-${payroll.employee_id}-${payroll.month}-${payroll.year}.pdf`);
    doc.pipe(res);
    ...
    if (attendance.regularization_summary?.approved_regularizations_applied !== undefined || summary.approved_regularizations_applied !== undefined) {
        line(doc, "Approved Regularizations Applied", attendance.regularization_summary?.approved_regularizations_applied ?? summary.approved_regularizations_applied);
    }
    line(doc, "Overtime Hours", attendance.overtime_breakdown
    ...
    if (totals.prorated_basic_salary !== undefined) {
        ...
        if ((attendance.proration_factor_percent ?? summary.proration_factor_percent) !== undefined
            ...
            doc.font("Helvetica").text(`× ${(attendance.proration_factor_percent ?? summary.proration_factor_percent ?? 0)}% (${(attendance.payable_days ?? summary.payable_days ?? 0)} payable / ${(period.working_days ?? summary.total_days ?? 0)} working days)`, { indent: 20 });
        }
    }
    componentLine(doc, { name: "Basic Salary", type: "fixed", value: totals.basic_salary, amount: totals.basic_salary });
    for (const component of components.allowances || earnings.allowances || [])
        componentLine(doc, component);
    for (const component of payableBonuses)
        componentLine(doc, component);
    componentLine(doc, {
        name: "Overtime",
        type: "fixed",
    ...
    for (const component of deductionsList)
        componentLine(doc, component);
    ...
    if (payableDaysForSummary > 0 && workingDaysForSummary > 0) {
        ...
        doc.fontSize(10).font("Helvetica").text(`(${money(dailyRate)} per working day × ${payableDays} payable ${dayLabel})`, { align: "center" });
    }
    doc.moveDown(0.8);
