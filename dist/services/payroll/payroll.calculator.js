...
export const calculatePayrollSnapshot = (employee, salaryStructure, period) => {
    ...
    ctx.allowancesTotal = round2(ctx.allowanceItems.reduce((sum, component) => sum + Number(component.amount || 0), 0));
    ...
    ctx.overtimeAmount = round2(ctx.overtimeHourlyRate * ctx.attendance.overtime_hours * Number(rules.overtime.multiplier) * overtimeProrationFactor);
    // Unapproved overtime is paid as regular working hours (no multiplier)
    ctx.unapprovedOvertimeAsRegularPay = round2(ctx.overtimeHourlyRate * ctx.attendance.unapproved_overtime_hours * overtimeProrationFactor);
    ctx.grossSalary = round2(ctx.basicSalaryProrated + ctx.allowancesTotal + ctx.bonusesTotal + ctx.overtimeAmount + ctx.unapprovedOvertimeAsRegularPay);
    ...
    ctx.nonTaxDeductionItems = nonTaxDeductions.map((component) => {
        ...
        if ((component.code === 'late_fine' || component.name?.toLowerCase().includes('late')) &&
    ...
    let nonTaxDeductionsTotal = round2(ctx.nonTaxDeductionItems.reduce((sum, component) => sum + Number(component.amount || 0), 0));
    ...
    let cappedAmount = 0;
    if (rules.deductions.cap_total_deductions_to_gross &&
        ...
        nonTaxDeductionsTotal = round2(ctx.nonTaxDeductionItems.reduce((sum, component) => sum + Number(component.amount || 0), 0));
    }
