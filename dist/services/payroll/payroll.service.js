...
import { fetchPayrollById, deletePayrollById, deletePayslipByPayrollId, findExistingPayroll, findPayslipByPayrollId, findSalaryStructureByEmployee, findSalaryStructureById, getEmployee, getEmployeesByIds, getSalaryStructure, insertSalaryStructure, listActiveSalaryStructureEmployeeIds, listPayrollByEmployee, listSalaryStructures, persistPayrollRow, updatePayrollStatus, updateSalaryStructureById, upsertPayslip, } from "./payroll.repository.js";
import { getAttendancePolicyById, getOvertimePolicyById, getTaxPolicyById, getBonusPolicyById } from "../policy.service.js";
...
const validateTaxPolicyForPayroll = (policy) => {
    if (!policy)
        throw error(400, "Tax policy is required");
    const mode = String(policy.tax_mode_default || "").toLowerCase();
    if (!mode)
        throw error(400, "Tax policy mode is required");
    if (mode === "slab") {
...
const validateBonusPolicyForPayroll = (policy) => {
    if (!policy)
        throw error(400, "Bonus policy is required");
    const mode = String(policy.bonus_mode_default || "").toLowerCase();
    if (!mode)
        throw error(400, "Bonus policy mode is required");
    if (mode === "percentage" || mode === "fixed") {
...
const normalizeComponent = (component = {}, fallbackName = "component", fallbackBasis = "basic_salary", defaultApplyProration = true) => {
    ...
    return {
        ...
        apply_proration: component.apply_proration === undefined
...
const normalizeComponentList = (components, fallbackName, fallbackBasis, defaultApplyProration = true) => {
    ...
    if (Array.isArray(components)) {
        return components.map((component, index) => normalizeComponent(component, `${fallbackName} ${index + 1}`, fallbackBasis, defaultApplyProration));
    }
    if (typeof components === "object") {
        return Object.entries(components).map(([name, component]) => normalizeComponent(typeof component === "object" ? { name, ...component } : { name, value: component }, name, fallbackBasis, defaultApplyProration));
    }
...
const toLegacySalaryResponse = (structure) => ({
    ...
    allowance_total: (structure.allowances || []).reduce((sum, component) => sum + Number(component.amount ?? component.value ?? 0), 0),
    deduction_total: (structure.deductions || []).reduce((sum, component) => sum + Number(component.amount ?? component.value ?? 0), 0),
    bonus_total: (structure.bonuses || []).reduce((sum, component) => sum + Number(component.amount ?? component.value ?? 0), 0),
});
...
const buildPayrollRow = (snapshot) => {
    const evaluated = snapshot?.attendanceSnapshot?.summary?.evaluated_attendance?.summary ||
...
export const updateSalaryStructureService = async (id, payload) => {
    try {
        ...
        return toSalaryStructureResponse(await attachLinkedPolicies(await updateSalaryStructureById(id, normalizeSalaryStructurePayload(payload))));
    }
    catch (e) {
        if (e.status)
            throw e;
...
export const getAllPayrollsService = async (query = {}) => {
    try {
        ...
        if (query.employee_id)
            payrollQuery = payrollQuery.eq("employee_id", query.employee_id);
        if (query.status)
