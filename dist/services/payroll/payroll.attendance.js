...
const groupAttendanceByDate = (attendanceRows, attendanceRules = {}) => {
    ...
    for (const row of attendanceRows || []) {
        if (!isBusinessDay(row.date, attendanceRules))
            continue;
...
const summarizeLeaveRequests = (leaveRows = []) => {
    ...
    for (const leave of leaveRows || []) {
        ...
        if (managerStatus === "approved")
            summary.manager_approved_requests += 1;
        if (hrStatus === "approved")
            summary.hr_approved_requests += 1;
        if (status === "rejected")
            summary.rejected_requests += 1;
        if (status === "approved" && isPaid)
            summary.approved_paid_requests += 1;
        if (status === "approved" && !isPaid)
            summary.approved_unpaid_requests += 1;
        if (status === "approved") {
...
const resolveShiftAssignmentByDate = (assignments = [], workingDates = []) => {
    ...
    const sortedAssignments = [...assignments].sort((a, b) => String(b.assigned_from || "").localeCompare(String(a.assigned_from || "")));
...
export const getPayrollPeriodSnapshot = async (employeeId, month, year, payrollRules = {}) => {
    ...
    const [{ data: attendanceRows, error: attendanceErr }, { data: leaveRows, error: leaveErr }, { data: overtimeRows, error: overtimeRowsErr }, { data: assignmentsRows, error: assignmentsErr },] = await Promise.all([
        supabase
            .from("attendance_records")
            .select("id, date, shift_id, check_in_time, check_out_time, status, duration_hours, overtime_hours, leave_override, notes, created_at, updated_at, shift:shift_id(id, name, start_time, end_time, duration_hours)")
    ...
    if (overtimeRowsErr)
        throw error(400, overtimeRowsErr.message);
    if (assignmentsErr)
        throw error(400, assignmentsErr.message);
    ...
    const payableOvertimeRows = (overtimeRows || []).filter((row) => String(row?.status || "").toLowerCase() === "approved" && Boolean(row?.is_paid === true));
    ...
    const regularizationByAttendanceId = await getApprovedRegularizationsForAttendanceIds((attendanceRows || []).map((row) => row.id).filter(Boolean));
    ...
    const potentialOvertimeRows = evaluateOvertimeFromAttendance(attendanceRows || [], shiftByDate, payrollRules?.overtime || {});
    ...
    const overtimeApprovalSplit = separateApprovedOvertimeByApproval(potentialOvertimeRows, payableOvertimeRows);
    ...
    const evaluationByDate = new Map((attendanceEvaluation?.evaluations || []).map((entry) => [String(entry.date), entry]));
    let presentDays = 0;
    let halfDays = 0;
    let halfDayUnits = 0;
    let paidLeaveDays = 0;
    ...
    const payableDays = round2((attendanceEvaluation?.evaluations || []).reduce((sum, row) => sum + Number(row?.payable_day_fraction || 0), 0));
    ...
    const totalShiftHours = round2(Array.from(bounds.workingDates).reduce((sum, day) => {
        const shift = shiftByDate.get(day);
        return sum + (shift?.shift?.duration_hours ? Number(shift.shift.duration_hours) : 8);
    }, 0));
    const totalWorkingHours = round2((attendanceEvaluation?.evaluations || []).reduce((sum, row) => sum + Number(row?.worked_hours || 0), 0));
    ...
    return {
        ...
        attendanceSummary: {
            ...
            late_penalty_rule: {
                ...
                computed_unpaid_days_from_late: Math.floor(lateArrivals / Number(payrollRules?.attendance?.late_count_for_unpaid_day || LATE_ARRIVAL_PENALTY_STEP)),
...
//# sourceMappingURL=payroll.attendance.js.map