...
import { getApprovedRegularizationsForAttendanceIds, resolveEffectiveAttendanceStatus, } from "./attendance/late-regularization.service.js";
...
const applyAttendanceFilters = (query, filters) => {
    ...
    if (filters.department)
        q = q.filter("employee.department", "eq", filters.department);
    return q;
};
...
const getRegularizationMapForAttendanceRows = async (rows = []) => getApprovedRegularizationsForAttendanceIds((rows || []).map((row) => row?.id).filter(Boolean));
const getEffectiveStatus = (row, regularizationMap = new Map()) => resolveEffectiveAttendanceStatus(row, regularizationMap.get(row?.id) || []).status;
...
const getPendingApprovals = async (filters) => {
    ...
    if (leaveRes.error)
        throw error(400, leaveRes.error.message);
    if (overtimeRes.error)
        throw error(400, overtimeRes.error.message);
    if (shiftRes.error)
        throw error(400, shiftRes.error.message);
    if (regularizationRes.error)
        throw error(400, regularizationRes.error.message);
    return {
