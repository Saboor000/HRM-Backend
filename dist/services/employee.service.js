...
const getUploadedUrl = async (files, key, folder) => files ? await uploadSingle(files, key, folder) : undefined;
...
const employeeByEmployeeId = async (employeeId) => supabase
    .from("employees")
    .select("id")
    .eq("employee_id", employeeId)
    .maybeSingle();
export const createEmployeeService = async ({ body, user, files }) => {
    const { email, password, designation, firstName, lastName, dob, gender, phone, address, employeeId, department, joiningDate, employmentType, emergencyName, emergencyPhone, } = body;
...
export const getAllEmployeesService = async (queryParams = {}) => {
    const { page = 1, limit = 10, search, role, department, employmentType, gender, sortBy = "created_at", sortOrder = "desc", } = queryParams;
    const from = (page - 1) * limit;
    const to = from + limit - 1;
    let query = supabase.from("employees").select("*", { count: "exact" }).is("deleted_at", null);
    if (role)
        query = query.eq("designation", role);
    if (department)
    ...
    if (search) {
        query = query.or(`first_name.ilike.%${search}%,last_name.ilike.%${search}%,employee_id.ilike.%${search}%,department.ilike.%${search}%,phone.ilike.%${search}%`);
    }
    query = query
...
export const updateEmployeeService = async ({ id, body = {}, files }) => {
    const { firstName, lastName, joiningDate, employmentType, emergencyName, emergencyPhone, role, dob, gender, phone, address, department, employeeId, } = body;
...
export const deleteEmployeeService = async ({ id }) => {
    const { data: employee, error: findError } = await employeeById(id, "id, auth_id, deleted_at", { includeDeleted: true });
    if (findError) {
