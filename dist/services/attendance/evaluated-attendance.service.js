...
const resolvePolicyTimezone = (attendancePolicy = {}) => resolveTimezone(attendancePolicy.timezone, process.env.ATTENDANCE_TIMEZONE, process.env.PAYROLL_POLICY_TIMEZONE, "Asia/Karachi");
const getLeaveUnit = (leave, leaveRules = {}) => {
    ...
    if (leave.leave_type === "short_leave") {
        ...
        return round2(Math.max(0, ((endHour * 60 + endMinute) - (startHour * 60 + startMinute)) / 60 / hoursPerDayForShortLeave));
    }
    return 1;
};
...
export const evaluateAttendanceRecord = ({ date, attendanceRecord = null, leaveRecord = null, attendancePolicy = {}, leaveRules = {}, shift = null, regularizations = [], }) => {
    ...
    const lateMinutes = hasCheckIn && shiftStartMinutes !== null && checkInMinutes !== null
    ...
    const fullDayHours = Number.isFinite(shiftBasedFullDayHours) && shiftBasedFullDayHours > 0
        ...
        : policyFullDayHours;
    const configuredHalfDayThreshold = Number(attendancePolicy?.min_hours_for_half_day
...
const groupAttendanceByDate = (attendanceRows = [], attendancePolicy = {}) => {
    ...
    for (const row of attendanceRows) {
        if (!row?.date)
            continue;
        if (!classifyBusinessDate(row.date, attendancePolicy || {}).isWorkingDay)
            continue;
...
export const evaluateAttendancePeriod = ({ attendanceRows = [], leaveRows = [], assignmentsRows = [], regularizationByAttendanceId = new Map(), periodBounds, attendancePolicy = {}, leaveRules = {}, }) => {
...
//# sourceMappingURL=evaluated-attendance.service.js.map