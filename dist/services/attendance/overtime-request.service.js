...
`;
const POLICY_TIMEZONE = resolveTimezone(process.env.PAYROLL_POLICY_TIMEZONE, process.env.ATTENDANCE_TIMEZONE, "Asia/Karachi");
...
const resolveOvertimePolicyForEmployee = async (employeeId) => {
    ...
    if (!policy)
        throw error(400, "Linked overtime policy not found");
    return policy;
};
const getExistingOvertimeHours = async (employeeId, date, monthRange) => {
    ...
    if (dailyRes.error)
        throw error(400, dailyRes.error.message);
    if (monthlyRes.error)
        throw error(400, monthlyRes.error.message);
...
export const createOvertimeRequestService = async (userId, payload) => {
    try {
        ...
        if (Math.abs(calculatedHours - requestedHours) > 0.1) {
            throw error(400, `Hours mismatch. Time difference: ${calculatedHours.toFixed(2)} hours`);
        }
...
export const managerOvertimeActionService = async (id, action, user, reason) => {
    ...
    if (overtime.status !== "pending")
        throw error(409, "This overtime request cannot be modified in its current state");
    if (overtime.manager_status !== "pending")
