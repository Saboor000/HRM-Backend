...
import { formatTimestampInTimezone, getDateInTimezone, resolveTimezone, toClockMinutesInTimezone, } from "../../utils/timezone.js";
...
const getEarlyCheckInWindowMinutes = (attendancePolicy = {}) => {
    const configuredWindow = Number(attendancePolicy?.early_check_in_window_minutes ?? DEFAULT_EARLY_CHECK_IN_WINDOW_MINUTES);
...
const enforceCheckInWindow = (shift, attendancePolicy, checkInTime) => {
    ...
    if (currentMinutes < earliestAllowedMinutes) {
        throw error(409, `Check-in is not allowed before the shift window opens (${earlyWindowMinutes} minute${earlyWindowMinutes === 1 ? "" : "s"} before shift start).`);
    }
};
...
const computeWorkMetrics = ({ checkInIso, checkOutIso, workedHours, shift, policy }) => {
    ...
    const shiftEndMinutes = shiftStartMinutes !== null && shiftEndRawMinutes !== null && shiftEndRawMinutes <= shiftStartMinutes
        ? shiftEndRawMinutes + 1440
        : shiftEndRawMinutes;
    const normalizeToShiftWindow = (minutes) => shiftStartMinutes !== null && minutes !== null && minutes < shiftStartMinutes ? minutes + 1440 : minutes;
    ...
    const lateMinutes = shiftStartMinutes !== null && checkInMinutes !== null
        ? Math.max(0, checkInMinutes - shiftStartMinutes)
        : 0;
    const earlyExitMinutes = shiftEndMinutes !== null && checkOutMinutes !== null
...
export const checkLeaveConflict = async (employeeId, date) => {
    try {
        const { data, error: err } = await supabase
            ...
            .eq("employee_id", employeeId)
            .eq("status", "approved")
            .or(`and(leave_type.eq.full_day,start_date.lte.${date},end_date.gte.${date}),` +
...
//# sourceMappingURL=checkin-checkout.service.js.map