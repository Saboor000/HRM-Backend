...
const assertNoConflictingAssignment = async ({ employeeId, excludeAssignmentId, }) => {
    const { data, error: err } = await supabase
        .from("employee_shift_assignments")
    ...
    if (conflict) {
        throw error(409, "Employee already has an active shift assignment. Deactivate the current assignment before creating a new one.");
    }
};
const resolveEmployeeId = async (employeeIdentifier) => {
    ...
    const { data: byId, error: byIdErr } = await supabase
        ...
        .maybeSingle();
    if (byIdErr)
        throw error(400, byIdErr.message);
    if (byId?.id)
        return byId.id;
    const { data: byAuth, error: byAuthErr } = await supabase
        ...
        .maybeSingle();
    if (byAuthErr)
        throw error(400, byAuthErr.message);
    if (!byAuth?.id)
        throw error(404, "Employee not found for provided employee_id");
    return byAuth.id;
};
