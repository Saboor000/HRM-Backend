...
const validateDurationHours = (providedDuration, startTime, endTime) => {
    ...
    if (Math.abs(roundedProvidedDuration - expectedDuration) > 0.01) {
        throw error(400, `duration_hours must match shift timing. Expected ${expectedDuration} for ${startTime} to ${endTime}`);
    }
};
...
export const deleteShiftService = async (id) => {
    try {
        ...
        if (attendanceCountErr)
            throw error(400, attendanceCountErr.message);
        if (assignmentCountErr)
            throw error(400, assignmentCountErr.message);
        if ((attendanceCount || 0) > 0) {
            if (shift.is_active) {
                ...
                return {
                    success: true,
                    action: "archived",
                    message: "Shift has attendance history and cannot be deleted. It has been deactivated instead.",
                    shift: archivedShift,
                };
            }
            return {
        ...
        if ((assignmentCount || 0) > 0) {
            if (shift.is_active) {
                throw error(409, "Cannot delete shift because it is linked to employee assignments. Deactivate it first, then delete it.");
            }
            const { error: deleteAssignmentsErr } = await supabase
                .from("employee_shift_assignments")
                .delete()
                .eq("shift_id", id);
            if (deleteAssignmentsErr)
                throw error(400, deleteAssignmentsErr.message);
        }
