...
const getLateRegularizationPolicy = async (employeeId) => {
    const { data: salaryStructure, error: salaryErr } = await supabase
        ...
        .maybeSingle();
    if (salaryErr)
        throw error(400, salaryErr.message);
    ...
    if (policyErr) {
        if (String(policyErr.message || "").toLowerCase().includes("does not exist")) {
            throw error(500, "Attendance policy regularization columns are missing. Run DB migration to add late_regularization_* fields.");
        }
...
const validateMonthlyLimit = async ({ employeeId, attendanceDate, limit, overrideAllowed }) => {
    if (overrideAllowed || !Number.isFinite(limit) || limit <= 0)
        return;
...
const hasType = (regularizations, values) => (regularizations || []).some((entry) => values.has(String(entry?.type || "").toLowerCase()));
...
export const submitLateRegularizationService = async (authId, payload = {}, files = []) => {
    ...
    const { data: duplicate, error: duplicateErr } = await supabase
        ...
        .maybeSingle();
    if (duplicateErr)
        throw error(400, duplicateErr.message);
    if (duplicate) {
...
export const listLateRegularizationsService = async (authId, query = {}) => {
    ...
    if (query.attendance_id)
        q = q.eq("attendance_id", query.attendance_id);
    if (query.status)
        q = q.eq("status", query.status);
    if (query.type)
        q = q.eq("type", String(query.type).toLowerCase());
    if (query.custom_type)
        q = q.ilike("custom_type", String(query.custom_type).trim());
    if (query.start_date)
        q = q.gte("incident_date", toDateOnly(query.start_date));
    if (query.end_date)
        q = q.lte("incident_date", toDateOnly(query.end_date));
...
export const reviewLateRegularizationService = async (authId, regularizationId, payload = {}) => {
    ...
    const { data: existing, error: fetchErr } = await supabase
        ...
        .maybeSingle();
    if (fetchErr)
        throw error(400, fetchErr.message);
    if (!existing)
        throw error(404, "Late regularization request not found");
...
//# sourceMappingURL=late-regularization.service.js.map