...
const normalizePolicyRecord = (tableName, record) => tableName === ATTENDANCE_TABLE ? normalizeAttendancePolicyRecord(record) : record;
...
const deletePolicy = async (tableName, id) => {
    ...
    if (linkedColumn) {
        ...
        if (countError)
            throw countError;
        if ((count || 0) > 0) {
            throw serviceError(409, `Cannot delete policy because it is linked to ${count} salary structure(s)`);
        }
    }
    const { data, error } = await supabase
        .from(tableName)
