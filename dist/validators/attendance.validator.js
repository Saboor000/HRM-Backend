import Joi from "joi";
import { hasInvalidTimeRange, hasSameTime, isoDateRule, limitRule, pageRule, parseFriendlyDate, strictObject, timeHHmmRule, uuidRule, } from "./common.validator.js";
...
const attendanceStatusRule = Joi.string()
    .valid("online", "offline", "absent", "holiday", "leave", "break", "PRESENT", "ABSENT", "ON_LEAVE", "ON_LEAVE_WORKING");
const requestStatusRule = Joi.string().valid("pending", "approved", "rejected", "cancelled");
const lateRegularizationTypeRule = Joi.string().valid("late_arrival", "missed_checkin", "early_checkout", "short_hours", "shift_mismatch", "system_error", "transport_issue", "weather_issue", "medical_reason", "official_work", "emergency", "other");
...
export const lateRegularizationSubmitSchema = strictObject({
    ...
    supporting_documents: Joi.alternatives().try(Joi.array().items(Joi.string().uri()), Joi.string().uri()),
}).custom((value, helpers) => {
...
"leaves", "overtime", "shifts", "assignments", "shift_requests", "late_regularization");
...
export const reportQuerySchema = strictObject({
    ...
    shift_id: uuidRule,
    // status filters
    status: Joi.string().trim().valid("pending", "approved", "rejected", "cancelled", "online", "offline", "absent", "holiday", "leave", "break", "PRESENT", "ABSENT", "ON_LEAVE", "ON_LEAVE_WORKING"),
    ...
    regularization_type: Joi.string().valid("late_arrival", "missed_checkin", "early_checkout", "short_hours", "shift_mismatch", "system_error", "transport_issue", "weather_issue", "medical_reason", "official_work", "emergency", "other"),
