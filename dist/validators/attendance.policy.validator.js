...
const dayToken = Joi.alternatives().try(Joi.number().integer().min(0).max(6), Joi.string().valid('sun', 'sunday', 'mon', 'monday', 'tue', 'tuesday', 'wed', 'wednesday', 'thu', 'thursday', 'fri', 'friday', 'sat', 'saturday'));
const stringListOrArray = (itemSchema) => Joi.alternatives().try(Joi.string().trim(), Joi.array().items(itemSchema));
const attendancePolicySchema = Joi.object({
    ...
    shift_grace_by_shift_name: Joi.object().pattern(Joi.string(), Joi.number().integer().min(0)).description('Override grace minutes for specific shifts by shift name'),
    shift_grace_by_shift_id: Joi.object().pattern(Joi.string(), Joi.number().integer().min(0)).description('Override grace minutes for specific shifts by shift id'),
...
//# sourceMappingURL=attendance.policy.validator.js.map