...
const run = async () => {
    try {
        ...
        const existingAdmin = (listedUsers?.users || []).find((user) => user?.app_metadata?.role === "admin" || user?.user_metadata?.role === "admin");
        if (existingAdmin) {
            console.log(`Admin already exists: ${existingAdmin.email}`);
            const employee = await ensureAdminEmployeeProfile(existingAdmin, existingAdmin.user_metadata?.name || name);
