...
const getCompactPayrollPayload = (data) => (data.payroll
...
const toCompactPayrollResponse = (payroll) => {
    const evaluatedAttendance = payroll?.attendance_snapshot?.summary?.evaluated_attendance?.summary ||
...
const enforceEmployeeSelfAccess = async (req, targetEmployeeId) => {
    if (req.user.designation !== "employee")
        return;
