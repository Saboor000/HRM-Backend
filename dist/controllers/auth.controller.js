...
const getAuthUserByIdOrResponse = async (res, id, notFoundMessage = "User not found") => {
    ...
    if (!data?.user)
        return { response: res.status(404).json({ message: notFoundMessage }) };
    return { data };
};
...
const createToken = (user, role = getRole(user)) => jwt.sign({ auth_id: user.id, email: user.email, role }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
export const signin = async (req, res, next) => {
    try {
        ...
        const { data: employee, error: employeeError } = await supabase
            ...
            .maybeSingle();
        if (employeeError)
            return badRequest(res, employeeError);
...
export const updateUserByAdmin = async (req, res, next) => {
    try {
        ...
        if (Object.keys(authUpdates).length > 0) {
            ...
            if (authUpdateError)
                return badRequest(res, authUpdateError);
        }
...
export const deleteUserByAdmin = async (req, res, next) => {
    try {
        ...
        if (authDeleteError)
            return badRequest(res, authDeleteError);
