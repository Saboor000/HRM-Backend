import { getDailyAttendanceReportService, getMyAttendanceReportService, getWeeklyAttendanceReportService, getMonthlyAttendanceReportService, getTeamSummaryReportService, } from "../../services/attendance/report.service.js";
...
const requireIsoDate = (value, label) => {
    ...
    if (!date)
        throw Object.assign(new Error(`Invalid ${label}. Use YYYY-MM-DD or DD-MM-YYYY`), { status: 400 });
    return date;
};
...
export const getMyAttendanceReport = async (req, res, next) => {
    try {
        ...
        send(res, 200, "My attendance report retrieved successfully", {
...
export const getAttendanceReport = async (req, res, next) => {
    try {
        ...
        if (type === "attendance") {
            ...
            if (subType === "daily") {
                const data = await getDailyAttendanceReportService(requireIsoDate(query.date, "date"), query.department);
                send(res, 200, "Daily attendance report retrieved successfully", data);
                return;
            }
            if (subType === "weekly") {
                const data = await getWeeklyAttendanceReportService(requireIsoDate(query.week_of, "week_of"), toInt(query.year));
                send(res, 200, "Weekly attendance report retrieved successfully", data);
                return;
            }
            if (subType === "monthly") {
                const data = await getMonthlyAttendanceReportService(toInt(query.month), toInt(query.year), query.department);
                send(res, 200, "Monthly attendance report retrieved successfully", data);
                return;
            }
            if (subType === "summary") {
                const data = await getTeamSummaryReportService(requireIsoDate(query.start_date, "start_date"), requireIsoDate(query.end_date, "end_date"), query.team_id);
        ...
        if (type === "overtime") {
            ...
            if (query.manager_status)
                filters.manager_status = query.manager_status;
            if (query.hr_status)
                filters.hr_status = query.hr_status;
