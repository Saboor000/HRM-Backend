import { createOvertimeRequestService, approveOvertimeRequestService, rejectOvertimeRequestService, managerOvertimeActionService, hrOvertimeActionService, cancelOvertimeRequestService, getOvertimeRequestsService, getOvertimeRequestByIdService, } from "../../services/attendance/overtime-request.service.js";
...
const getListFilters = (query, keys) => keys.reduce((acc, key) => {
...
export const getMyOvertimeRequests = async (req, res, next) => {
    try {
        ...
        const data = await getOvertimeRequestsService({ ...filters, employee_id: employee.id }, page, limit);
...
export const managerOvertimeAction = async (req, res, next) => {
    try {
        const data = await managerOvertimeActionService(req.params.id, req.body.action, req.user, req.body.rejection_reason);
...
export const hrOvertimeAction = async (req, res, next) => {
    try {
        const data = await hrOvertimeActionService(req.params.id, req.body.action, req.user, req.body.rejection_reason);
