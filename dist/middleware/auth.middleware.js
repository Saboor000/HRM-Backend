...
const extractBearerToken = (authorization = "") => authorization.startsWith("Bearer ") ? authorization.split(" ")[1] : null;
...
export const protect = (req, res, next) => {
    ...
    try {
        ...
        if (decoded?.auth_id && !decoded?.id)
            decoded.id = decoded.auth_id;
        if (decoded?.id && !decoded?.auth_id)
            decoded.auth_id = decoded.id;
        req.user = decoded;
        return next();
    }
    catch (error) {
