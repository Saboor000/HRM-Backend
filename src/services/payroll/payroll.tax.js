...
export const normalizeTaxSlabs = (slabs = []) => {
  const normalized = (slabs || [])
    .map((slab) => ({
      up_to:
        slab?.up_to === null || slab?.up_to === undefined || slab?.up_to === ""
          ? null
          : Number(slab.up_to),
...
export const resolveTaxDeduction = ({
  taxComponent,
  basicSalary,
  grossSalary,
  preferredMode = "slab",
  ...
  if (
    taxComponent?.tax_mode === "slab" ||
    Array.isArray(taxComponent?.tax_slabs)
  ) {
    const slabs = normalizeTaxSlabs(
      taxComponent?.tax_slabs || policyTaxSlabs || DEFAULT_MONTHLY_TAX_SLABS
    );
    const amount = calculateSlabTax(grossSalary, slabs);
