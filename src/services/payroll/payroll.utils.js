...
const toDayIndex = (value) => {
  ...
  if (typeof value === "string") {
    ...
    if (WEEKDAY_NAME_TO_INDEX[trimmed] !== undefined) return WEEKDAY_NAME_TO_INDEX[trimmed];
  }
  return null;
};
...
const getAttendanceCalendarConfig = (attendanceRules = {}) => {
  const weeklyOffDays = parseDayIndexSet(
    attendanceRules.weekly_off_days ?? process.env.PAYROLL_WEEKLY_OFF_DAYS,
    [0, 6]
  );

  const workingWeekendDates = toDateSet(
    attendanceRules.working_weekend_dates ?? process.env.PAYROLL_WORKING_WEEKEND_DATES
  );

  const holidayDates = toDateSet(
    attendanceRules.holiday_dates ?? process.env.PAYROLL_HOLIDAY_DATES
  );

  const forcedWorkingDates = toDateSet(
    attendanceRules.forced_working_dates ?? process.env.PAYROLL_FORCED_WORKING_DATES
  );

  const manualOffDates = toDateSet(
    attendanceRules.manual_off_dates ?? process.env.PAYROLL_MANUAL_OFF_DATES
  );
...
export const classifyBusinessDate = (dateValue, attendanceRules = {}) => {
  ...
  if (!date) return { isWorkingDay: false, reason: "invalid_date" };
...
export const getComponentTotal = (components, baseValues) =>
  round2((components || []).reduce((sum, component) => sum + getComponentAmount(component, baseValues), 0));
