...
export const getSalaryStructure = async (employeeId) => {
  ...
  if (err) throw error(400, err.message);
  if (!data) throw error(404, "Salary structure not found for employee");
  return data;
};
...
export const findSalaryStructureById = async (id) => {
  ...
  if (qErr) throw error(400, qErr.message);
  if (!data) throw error(404, "Salary structure not found");
  return data;
};
...
export const fetchPayrollById = async (id) => {
  ...
  if (err) throw error(400, err.message);
  if (!data) throw error(404, "Payroll not found");
  return data;
};
...
export const listPayrollByEmployee = async (employeeId, query = {}) => {
  ...
  if (query.month && query.year) {
    ...
    if (err) throw error(400, err.message);
    if (!data) throw error(404, "Payroll not found for the selected period");
